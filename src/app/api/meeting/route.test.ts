...
describe('/api/meeting/[meetingCode]', () => {
  beforeEach(() => {
    ...
    vi.mock('@google-cloud/tasks', () => {
      ...
      CloudTasksClient.prototype.createTask = vi.fn(() => Promise.resolve([{ name: 'task123' }]));
  ...
  describe('POST', () => {
    it('should return 400 for invalid request body', async () => {
      ...
      await expectBadRequest(response, { path: ['scheduledEndTime'], message: 'Required' });
    });
    ...
    it('should fail if it is not a date', async () => {
        ...
        const response = await handler.POST(req);
        await expectBadRequest(response, { path: ['scheduledEndTime'], message: 'Invalid datetime' });
      });

    it('should fail if time is not far enough in the future', async () => {
        const req = new NextRequest('http://localhost/api/meeting', {
          method: 'POST',
          body: JSON.stringify({ scheduledEndTime: formatISO(add(new Date(), {minutes: 2}))}),
        });
  
        const response = await handler.POST(req);
        await expectBadRequest(response, { path: ['scheduledEndTime'], message: 'Datetime must be 5 minutes in future' });
    });

    it('should return 403 if no credentials in session', async () => {
      const req = new NextRequest('http://localhost/api/meeting', {
        method: 'POST',
        body: JSON.stringify({ scheduledEndTime: formatISO(add(new Date(), {minutes: 10}))}),
      });
    ...
    it('should return 500 if meeting creation fails', async () => {
      ...
      expect(await response.text()).toBe('Creation of meeting failed: Google API error');
    });
    ...
    it('should create a meeting and return the meeting data', async () => {
      ...
      vi.mocked(CloudTasksClient.prototype.createTask).mockResolvedValue([{ name: 'task123' }] as any);
...
async function expectBadRequest(response: NextResponse<unknown>, error: Partial<ZodIssue>) {
